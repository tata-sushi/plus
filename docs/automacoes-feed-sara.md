...
| matrícula | nome            | status | papel                          |
|-----------|-----------------|--------|--------------------------------|
| `bot1`    | Sara            | `Bot`  | publica aniversário/boas-vindas |
| `bot2`    | Tatá Bastidores | `Bot`  | reservado (não usado ainda)    |
...
- **Lógica:** pega quem é `status='Ativo'` e `data_demissao is null` e faz aniversário
  hoje (dia+mês); monta a menção de todos, sorteia 1 frase de
  `tata_plus.aniversario_mensagens` (tipo `nascimento`, ativas — 5 hoje) e publica 1 post
  como `bot1` (`tipo='aniversario'`). Sem aniversariante → não publica.
- **Idempotência:** não repete se já houver post `tipo='aniversario'` de `bot1` no dia (SP).
...
- **Lógica:** pega admitidos na **semana passada** (seg–dom), `Ativo` e sem demissão,
  ...
  **carrossel** (`midias`) + texto com menção e "Departamento · Unidade" por pessoa;
  publica como `bot1` (`titulo='Boas-vindas ao time! 👋💚'`, `tipo='boas_vindas'`).
...
-- avatar da Sara
update tata_plus.auth_users set avatar_url='<url pública>' where matricula='bot1';
```
