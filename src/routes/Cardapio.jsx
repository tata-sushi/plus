    ...
    <>
      ...
      <div className="mt-2 px-5">
        <div className="hstack gap-2 text-xs font-semibold text-muted">
          ...
          Cardápio da semana · {fmtDia(datas[0])} – {fmtDia(datas[6])}
        </div>
      </div>

      <Section className="mt-4">
        <div className="flex flex-col gap-3">
          {cardapioSemanal.map((dia, i) => {
            const isHoje = i === idxHoje
            return (
              <Card key={dia.abrev} highlight={isHoje} className="reveal">
                <div className="hstack justify-between">
                  <div>
                    <div className="font-display text-base font-bold">{dia.nome}</div>
                    <div className="text-[11px] text-muted">{fmtDia(datas[i])}</div>
                  </div>
                  {isHoje && (
