...
// Seletor de quem pode ver os VALORES (financeiros) nas páginas de governança.
// Grava em dp_rh.perm_ver_valores (liberação global, area='geral') pelas RPCs
// admin perm_valores_listar / perm_valores_set. Como é um único liga/desliga por
// pessoa, o toggle salva na hora — não precisa de modal nem botão de salvar.
export function AdminVerValores() {
  ...
  const comAcesso = (pessoas || []).filter((p) => p.pode_ver).length

  async function alternar(pessoa) {
    if (salvando) return
    const novo = !pessoa.pode_ver
    setErro('')
    setSalvando(pessoa.matricula)
    tapHaptic()
    // otimista: reflete na hora e reverte se a RPC falhar.
    setPessoas((prev) =>
      prev.map((p) => (p.matricula === pessoa.matricula ? { ...p, pode_ver: novo } : p)),
    )
      ...
      <div className="px-5 pt-4">
        <Card className="hstack gap-3">
          ...
          <div className="text-xs leading-relaxed text-muted">
            Quem estiver <b className="text-text">ligado</b> passa a ver os{' '}
            <b className="text-text">valores financeiros</b> nas páginas de governança. Desligado, a
            pessoa continua acessando as páginas, mas os valores ficam ocultos.
          </div>
        </Card>
